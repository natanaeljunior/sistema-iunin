...
@Component({
  templateUrl: 'parametro-analise-list.component.html',
  styleUrls: ['parametro-analise-list.component.scss'],
  providers:[DataService]
})

export class ParametroAnaliseListComponent {
  @ViewChild('dataGridParametroAnalise') dataGrid: DxDataGridComponent;

  resource: string= 'Parametro Analise';
  ufRegex = /^[A-Z]{2}$/i;
  ...
  parametroAnaliseColumns: any[] = [
    {
      dataField: 'nome_parametro',
      caption: 'Nome Parametro',
      dataType: 'string',
      validationRules: [
        {
          type: 'required',
    ...
    {
      dataField: 'valor_referencia',
      caption: 'Valor Referência',
      dataType: 'number',
      validationRules: [
        {
          type: 'required',
  ...
  constructor(private apollo: Apollo, private dataService: DataService) {
    ...
    this.dataSource = new DataSource({
      key: 'id',
      load: () => this.dataService.get('findAllParametrosAnalises').then((response) => {
        const data = response.parametros_analises;
        return {
          data,
      ...
      insert: (object): any => {
        debugger
        this.dataService.post('newParametroAnalise', {object}).then((response) => {
      ...
      update: (key, object): any => {
        this.dataService.patch('updateParametroAnalise/' + key, {object}).then((response) => {
      ...
      remove: (key): any => {
        this.dataService.delete('deleteParametroAnalise/' + key).then((response) => {
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [ParametroAnaliseListComponent],
})
